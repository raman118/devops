...
# 1. SERIALIZATION: Python object → YAML string
python_data = {
    'name': 'John Doe',
    'age': 30,
...
print(f"First skill: {python_object['skills'][0]}")
```
...
for env in environments:
    backend_config = generate_terraform_backend_config(
        bucket=f'terraform-state-{env}',
        key=f'infrastructure/{env}/terraform.tfstate',
        region='us-west-2',
...
with open('data_pipeline.yaml', 'w') as f:
    f.write(pipeline_yaml)

pipeline_config = DataPipelineConfig('data_pipeline.yaml')

print("Pipeline Sources:")
for source in pipeline_config.get_sources():
...
def demonstrate_memory_usage():
    ...
    print(f"Original data size in memory: {sys.getsizeof(large_data)} bytes")
    
    # ❌ Memory inefficient - loads everything at once
    yaml_string = yaml.dump(large_data)
    print(f"YAML string size: {len(yaml_string)} characters")
    
    # ✅ Memory efficient - streaming approach
    def write_large_yaml_streaming(data, filename):
    ...
    print("Large YAML file written efficiently using generator")
...
class OptimizedYAMLHandler:
    def __init__(self):
        ...
        try:
            ...
            self.Loader = Loader
            self.Dumper = Dumper
            print("Using Python YAML loader (slower)")
    
    def fast_load(self, yaml_string: str) -> Any:
        """Load YAML with optimized settings"""
        return yaml.load(yaml_string, Loader=self.Loader)
    ...
    def benchmark_loading(self, yaml_content: str, iterations: int = 1000):
        ...
        print(f"Standard loading: {standard_time:.4f} seconds")
        print(f"Optimized loading: {optimized_time:.4f} seconds")
...
yaml_handler.benchmark_loading(test_yaml)
```

## Error Handling and Debugging

### 1. Common YAML Errors and Solutions
...
def debug_yaml_errors(yaml_content: str) -> Tuple[bool, Optional[dict], Optional[str]]:
    ...
    try:
        ...
        error_msg = f"Unknown YAML Error: {e}"
        print(f"❌ {error_msg}")
        return False, None, error_msg
...
for i, yaml_content in enumerate(yaml_errors, 1):
    ...
    if success:
        print("✅ Valid YAML")
        print(f"Parsed data keys: {list(data.keys()) if data else 'None'}")
    else:
...
class YAMLValidator:
    ...
    def validate_file(self, filepath: str) -> bool:
        """Validate YAML file comprehensively"""
        self.errors.clear()
        self.warnings.clear()
        ...
        try:
            ...
            self.errors.append(f"Failed to read or parse file: {e}")
            return False
